...
def eval_algorithm( train, test, key, algorithm, eval, metrics, results, conf, slice=None, iteration=None, out=True ):
    ...
    if hasattr(algorithm, 'evaluate_dl') and algorithm.evaluate_dl:
        conf_metrics_multiple = [{'class': 'accuracy_multiple.Precision', 'length': [20]},
                        {'class': 'accuracy_multiple.Recall', 'length': [20]},
                        {'class': 'accuracy_multiple.MAP', 'length': [20]}]
        metrics_multiple = create_metric_list(conf_metrics_multiple)
        results[key] = algorithm.evaluate_from_dataloader(metrics, metrics_multiple)
    else:
